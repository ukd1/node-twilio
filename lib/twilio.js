...
//console.log('answered uri',answeredUri);   
...
PhoneNumber.prototype.makeCall = function(to, opts, fn) {
	...
	logger.debug('Make call prototype \n');
	logger.debug(call);
};
...
// moved the demo account function down here to make it clear that all it does is get the numbers from the outgoing number set as opposed to the incoming. They share all other functions.


/**
 * getOutgoingPhoneNumber: Return a new OutgoingPhoneNumber object aka  the demo account version
 *
 * @param {String} num: The phone number or phone number sid.
 */
Client.prototype.getOutgoingPhoneNumber = function(num, opts) {
    return new OutgoingPhoneNumber(this, num, opts);
};

/**
 * OutgoingPhoneNumber class: Represents an incoming phone number
 *
 * @param {Object} client: An instantiated RestClient object
 * @param {String} num: The phone number/phone number's sid
 */
function OutgoingPhoneNumber(client, num, opts) {
    var self = this;
    
    if(!(this instanceof OutgoingPhoneNumber)) {
        return new OutgoingPhoneNumber(client, num);
    }
...
/*
    this.on('newListener', function(listener) {
        ...
        self.client.updateOutgoingNumber(self.attrs.sid, update);
    });
*/
};

util.inherits(OutgoingPhoneNumber, EventEmitter);
 ...
 * @param {Function} fn: Callback for completion
 */
OutgoingPhoneNumber.prototype.getNumberDetails = function(fn) {
    var self = this;
...
//        self.attrs.smsFallbackMethod = numberDetails.sms_fallback_method;
    ...
    if(self.sid) {
        self.client.getOutgoingNumber(self.sid, function(resp) {
            populate(resp);
        });
    } else {
        // Passed in an actual number, gotta look it up.
        self.client.getOutgoingNumbers({PhoneNumber: self.attrs.phoneNumber},
            function(resp) {

	//console.log(resp);       
        var num = resp && resp.outgoing_caller_ids && 
                    resp.outgoing_caller_ids[0];
	...
	logger.debug('Outgoing Phone Number attrs after get details \n');
	logger.debug(self);
};


/**
 * setup: Configure the outgoing number to start emitting events
 *
 * @param {Function} fn: Called on completion
 */
OutgoingPhoneNumber.prototype.setup = function(fn) {
    var self = this;
...
OutgoingPhoneNumber.prototype.makeCall = function(to, opts, fn) {
    var call = new OutgoingCall(this.attrs.phoneNumber, to, opts, this.client);
    call.setup(fn);
};
